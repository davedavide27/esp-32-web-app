...
const Dashboard = () => {
  ...
  const [lastSavedUpdate, setLastSavedUpdate] = useState(null);
  ...
  const [ledPending, setLedPending] = useState({
    led1: false,
    led2: false,
    led3: false,
  });
  ...
  const latestData =
    realTimeData.length > 0 ? realTimeData[realTimeData.length - 1] : null;

  const hasManualButtonActive = useMemo(() => {
    ...
    // Stale connection detection effect
    useEffect(() => {
      const STALE_TIMEOUT = 10000;
      const STALE_CHECK_INTERVAL = 5000;
      const interval = setInterval(() => {
        if (lastLiveUpdate && Date.now() - lastLiveUpdate.getTime() > STALE_TIMEOUT) {
          showNotification("Connection stale: attempting to reconnect...", "warning");
  ...
  const effectiveAutomationEnabled = useMemo(() => {
    if (latestData && latestData.controlMode === "automatic") {
      // In automatic mode, always reflect backend automation state
      if (typeof latestData.automationEnabled === "boolean") {
        return latestData.automationEnabled;
      }
      return automationEnabled;
    }
    // In manual mode, button state can override
    if (hasManualButtonActive) return false;
    if (latestData && typeof latestData.automationEnabled === "boolean") {
      return latestData.automationEnabled;
    }
    return automationEnabled;
  }, [latestData, hasManualButtonActive, automationEnabled]);

  const effectiveManualOverrideActive = useMemo(() => {
    if (hasManualButtonActive) return true;
    if (latestData && typeof latestData.manualOverrideActive === "boolean") {
      return latestData.manualOverrideActive;
    }
    return manualOverrideActive;
  }, [latestData, hasManualButtonActive, manualOverrideActive]);
  ...
  const fetchLastSavedUpdate = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/last-update`);
      const data = await response.json();
      if (data.lastUpdate) {
        setLastSavedUpdate(new Date(data.lastUpdate));
      }
    } catch (error) {
      console.error("Error fetching last saved update:", error);
    }
  };
  ...
  const sendButtonCommand = async (buttonIdx) => {
    try {
      // Send button press command
      await fetch(`${BACKEND_URL}/api/command`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: `button${buttonIdx}_press` }),
      });
        showNotification(`Button ${buttonIdx} pressed.`, "info");
    } catch (err) {
      showNotification("Failed to send button command", "error");
      console.error("Error sending button command:", err);
    }
  };

  const sendAutomationToggle = async (enabled) => {
    try {
      ...
      if (resp.ok) {
          ...
          // Always reset manual override when toggling automation
          setManualOverrideActive(false);
          // Optionally clear button states if needed
        showNotification(
          enabled ? "Automatic control enabled" : "Automatic control disabled",
          "info"
        );
      } else {
        showNotification("Failed to update automation mode", "error");
      }
    } catch (error) {
  ...
  const refreshData = () => {
    fetchHistoricalData();
    fetchLastSavedUpdate();
  };

  const formatDataForChart = (data) => {
    return data.map((item) => {
      const radarValue =
        item.ld2410cHumanPresent == null
          ? 0
          : parseInt(item.ld2410cHumanPresent, 10);
      ...
      return {
        ...
        humidity1:
          item.humidity1 == null || isNaN(item.humidity1)
            ? 0
            : parseFloat(item.humidity1),
        voltage:
          item.voltage == null || isNaN(item.voltage)
            ? 0
  ...
  const getCurrentValues = () => {
    ...
    return {
      temperature: `${
        latest.temperature == null || isNaN(latest.temperature)
          ? 0
          : latest.temperature
      }°C`,
      humidity1: `${
        latest.humidity1 == null || isNaN(latest.humidity1)
          ? 0
          : latest.humidity1
      }%`,
      voltage: `${
        latest.voltage == null || isNaN(latest.voltage) ? 0 : latest.voltage
      }V`,
      ld2410cHumanPresent:
        parseInt(latest.ld2410cHumanPresent, 10) === 1
          ? "Human Present"
          : "No Presence",
  ...
  const exportCSV = () => {
    ...
    const header = "Time,Temperature (°C),Humidity 1 (%),Voltage (V),Radar\n";
    const rows = realTimeData
      .map(
        (d) =>
          `${new Date(d.timestamp).toLocaleString()},${d.temperature},${
            d.humidity1
          },${d.voltage},${d.ld2410cHumanPresent}`
      )
      .join("\n");
  ...
  const exportPNG = async () => {
    ...
    canvas.toBlob((blob) => {
      if (blob) saveAs(blob, "sensor_chart.png");
    });
  };
  ...
  useEffect(() => {
    ...
    newSocket.on("sensorData", (data) => {
      setRealTimeData((prevData) => [...prevData.slice(-99), data]);
      setIsLoading(false);
      setEsp32Status("active");
      setLastLiveUpdate(new Date());

      // Only update automationEnabled and manualOverrideActive if controlMode is present
      if (data.controlMode === "automatic") {
        setAutomationEnabled(true);
        setManualOverrideActive(false); // Always false in automatic mode
      } else if (data.controlMode === "manual") {
        setAutomationEnabled(false);
        // Only set manual override active if a button is pressed
        ...
        } else if (data.button1 || data.button2 || data.button3 || data.button4) {
          setAutomationEnabled(false);
        }
        if (typeof data.manualOverrideActive === "boolean") {
          setManualOverrideActive(data.manualOverrideActive);
        } else if (data.button1 || data.button2 || data.button3 || data.button4) {
          setManualOverrideActive(true);
        } else {
          setManualOverrideActive(false);
      ...
      if (
        typeof data.led1 !== "undefined" ||
        typeof data.led2 !== "undefined" ||
        typeof data.led3 !== "undefined"
      ) {
        const nextStates = {
          led1: !!data.led1,
          led2: !!data.led2,
        ...
        setLedStates(nextStates);
        setLedPending({ led1: false, led2: false, led3: false });
      }
    ...
    newSocket.on("savedSensorData", (data) => {
      ...
      setLastSavedUpdate(new Date());
    });

    newSocket.on("esp32Error", (payload) => {
      if (payload && payload.message) {
        showNotification(`ESP32 Error: ${payload.message}`, "error");
        console.error("ESP32 Error:", payload.message);
      }
    });
    ...
    const handleBeforeUnload = (e) => {
      e.preventDefault();
      e.returnValue =
        "Are you sure you want to leave? Live sensor data will be lost.";
    };
    ...
    <div className={`dashboard-root ${theme === "dark" ? "dark" : ""}`}>
      <div className="dashboard-container">
        ...
        <section className="dashboard-status-bar">
          <div
            className={`dashboard-status ${
              esp32Status === "active" ? "status-online" : "status-offline"
            }`}
          >
            {esp32Status === "active" ? (
              <>
          ...
          <div className="dashboard-status-info">
            ...
            <span>{lastLiveUpdate ? lastLiveUpdate.toLocaleString() : "Never"}</span>
          </div>
          ...
          <div className="dashboard-status-info">
            <span>Controls:</span>
            <button className="chart-action-btn" onClick={() => setControlsOpen(true)}>
              Open Controls
            </button>
          </div>
        </section>
          ...
          >
            ...
            >
              ...
              <div style={{ textAlign: "right", marginTop: 12 }}>
                <button className="chart-action-btn" onClick={() => setControlsOpen(false)}>
                  Close
                </button>
              </div>
            </div>
        ...
        <section className="dashboard-current">
          ...
          <div className="dashboard-current-card">
            <span className="current-icon fan" role="img" aria-label="Fan">
              🌰
            </span>
            <div>
              <div className="current-label">Fan Control</div>
              <div className="current-value fan">
                {effectiveAutomationEnabled ? "Automatic" : "Manual"}
                <button
                  style={{
                    marginLeft: 8,
                    padding: "4px 12px",
                    borderRadius: "6px",
                    background: effectiveAutomationEnabled
                      ? "#22c55e"
                      : effectiveManualOverrideActive
                      ? "#e5e7eb"
                      : "#ef4444",
                    color: effectiveManualOverrideActive ? "#9ca3af" : "#fff",
                    border: "none",
                    fontWeight: 600,
                    cursor: effectiveManualOverrideActive ? "not-allowed" : "pointer",
                  }}
                  onClick={() => sendAutomationToggle(!effectiveAutomationEnabled)}
                    disabled={false}
                >
                  {effectiveAutomationEnabled ? "Disable" : "Enable"} Auto
                </button>
              </div>
              <div
                className="current-value fan"
                style={{ fontSize: "0.9em", color: "#6b7280", marginTop: 4 }}
              >
                {effectiveManualOverrideActive
          ...
          <div className="dashboard-current-card">
            <span className="current-icon" role="img" aria-label="LD2410C">
              🛰️
            </span>
            <div>
          ...
          <div className="dashboard-current-card">
            <span
              ...
              style={{
                background: (() => {
                  if (currentValues.button1 === "ON" || ledStates.led1) return "#bbf7d0";
                  if (currentValues.button2 === "ON" || ledStates.led2) return "#fef9c3";
                  if (currentValues.button3 === "ON" || ledStates.led3) return "#fecaca";
                  return "#e5e7eb";
                })(),
                color: (() => {
                  if (currentValues.button1 === "ON" || ledStates.led1) return "#22c55e";
                  if (currentValues.button2 === "ON" || ledStates.led2) return "#eab308";
                  if (currentValues.button3 === "ON" || ledStates.led3) return "#ef4444";
                  return "#6b7280";
                })(),
                fontWeight: 700,
                fontSize: "2rem",
        ...
        <section className="dashboard-chart-section">
          <div className="dashboard-chart-header">
            ...
            <div className="dashboard-chart-exports">
              ...
              <input
                ...
                className="chart-date-input"
              />
              <button onClick={applyDateFilter} className="chart-action-btn">
                Apply
              </button>
              <button onClick={exportCSV} className="chart-action-btn">
                Export CSV
              </button>
              <button onClick={exportPNG} className="chart-action-btn">
                Export PNG
              </button>
              <button onClick={refreshData} className="chart-action-btn">
                <RefreshCw className="chart-action-icon" />
                Refresh
              </button>
              <button onClick={clearData} className="chart-action-btn danger">
                <Trash2 className="chart-action-icon" />
                Clear
              </button>
            </div>
          </div>
          ...
          {(() => {
            ...
            const chartTotalPages = Math.max(
              1,
              Math.ceil(savedData.length / chartPageSize)
            );
            const [chartPage, setChartPage] = React.useState(1);
            const [animating, setAnimating] = React.useState(false);

            const chartFormattedData = formatDataForChart(
              [...savedData].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
            );

            const chartPageData = chartFormattedData.slice(
              (chartPage - 1) * chartPageSize,
              chartPage * chartPageSize
            );

            const handleChartPageChange = (newPage) => {
              if (newPage < 1 || newPage > chartTotalPages || newPage === chartPage) {
                return;
              }
              setAnimating(true);
              setTimeout(() => {
              ...
              <>
                  ...
                  >
                    {isLoading ? (
                      ...
                      <div className="dashboard-nodata">
                        No data available. Waiting for ESP32 connection...
                      </div>
                    ) : (
                ...
                <div className="example small" style={{ marginTop: 8, marginBottom: 4 }}>
                  <div
                    className="text"
                    style={{
                      cursor: chartPage > 1 ? "pointer" : "not-allowed",
                      color: chartPage > 1 ? "#6a558e" : "#bbb",
                    }}
                    onClick={() => handleChartPageChange(chartPage - 1)}
                  >
                    previous data
                  </div>

                  <div className="counter" style={{ position: "relative" }}>
                    <span className="number" style={{ fontFamily: "Rowdies, cursive" }}>
                      {String(chartPage).padStart(2, "0")}
                    </span>
                    <div className="background"></div>
                    <span className="number" style={{ fontFamily: "Rowdies, cursive" }}>
                      {String(chartTotalPages).padStart(2, "0")}
                    </span>
                  </div>
                  ...
                  <div
                    className="text"
                    style={{
                      cursor: chartPage < chartTotalPages ? "pointer" : "not-allowed",
                      color: chartPage < chartTotalPages ? "#6a558e" : "#bbb",
                    }}
                    onClick={() => handleChartPageChange(chartPage + 1)}
                  >
                    next data
        ...
        <section className="dashboard-table-section">
          <h3 className="dashboard-table-title">
            <Database className="table-title-icon" />
            Recent Sensor Data
          </h3>
                  ...
                  {currentPageData.map((data, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          {data.ld2410cHumanPresent === 1 ||
                          data.ld2410cHumanPresent === true ||
                          data.ld2410cHumanPresent === "true"
                            ? "Yes"
                            : "No"}
                        </td>
                        <td>{data.controlMode ? (data.controlMode.charAt(0).toUpperCase() + data.controlMode.slice(1)) : "Automatic"}</td>
                      </motion.tr>
                    );
                  })}
                </tbody>
